...
# ── per-component knobs ──────────────────────────────────────────────────────
ID=lumen-applications-menu       # herald package id (== repo name)
NAME="Applications menu"         # human-readable name
DESTBIN=bin/applications         # where the binary installs (verbatim runtime path)
